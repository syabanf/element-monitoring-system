...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const asset = await prisma.asset.findUnique({
    ...
    include: {
      ...
      sensors: { select: { id: true, name: true, sensorType: true, metricName: true, unit: true, status: true, lastValue: true, lastReadingAt: true } },
      alerts: { where: { status: { in: ["OPEN","ACKNOWLEDGED","INVESTIGATING"] } }, select: { id: true, title: true, severity: true, status: true, openedAt: true }, take: 5 },
