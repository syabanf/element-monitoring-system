...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EE] flex">
      {/* Left panel — branding */}
      <div className="hidden lg:flex flex-col justify-between w-96 bg-[#1C1714] p-10 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-[#9C9285] text-sm leading-relaxed">
        ...
        <div className="relative z-10 space-y-3">
          {["7 Utility Pillars", "Real-time Telemetry", "Compliance Reporting", "RBAC Access Control"].map((f) => (
            <div key={f} className="flex items-center gap-2">
              ...
              <span className="text-[#9C9285] text-xs">{f}</span>
            </div>
          ))}
          <p className="text-[#5C5248] text-xs mt-4 pt-3 border-t border-[#2a2520]">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="lg:hidden flex items-center gap-3 mb-2">
            ...
            <div>
              <div className="text-[#1C1714] font-bold text-base tracking-wider">ELEMENT</div>
              <div className="text-[#B8901A] text-[9px] font-semibold tracking-widest uppercase">Monitoring System</div>
            </div>
          </div>

          <div>
            <h1 className="text-[#1C1714] text-2xl font-bold">Welcome back</h1>
            <p className="text-[#9C9285] text-sm mt-1">Sign in to your monitoring account</p>
          </div>

          {/* Form */}
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-[#5C5248] mb-1.5 uppercase tracking-wider">
                Email Address
              </label>
              <input
                {...register("email")}
                ...
                className="w-full bg-white border border-[#E5DDD0] rounded-xl px-4 py-3 text-[#1C1714] text-sm placeholder-[#C4B9AD] focus:outline-none focus:border-[#B8901A] focus:ring-2 focus:ring-[#B8901A]/15 transition"
              />
          ...
          {/* Demo Credentials */}
          <div className="bg-white border border-[#E5DDD0] rounded-xl p-5 space-y-3 shadow-sm">
            <div className="flex items-center gap-2">
              <Activity className="w-3.5 h-3.5 text-[#B8901A]" />
              <p className="text-[#5C5248] text-xs font-bold uppercase tracking-wider">Demo Accounts</p>
            </div>
            <div className="space-y-1.5">
              {demoUsers.map((u) => (
                <button
                  ...
                  className="w-full text-left flex items-center justify-between bg-[#F5F3EE] hover:bg-[#FEF7E6] border border-[#EDE8E0] hover:border-[#F5E6B5] rounded-lg px-3 py-2 transition-all group"
                >
                  <div>
                    <div className="text-[#1C1714] text-xs font-semibold">{u.email}</div>
            ...
            <p className="text-[#C4B9AD] text-[10px]">Password for all accounts: <strong className="text-[#9C9285]">password123</strong></p>
          </div>
        </div>
      </div>
    </div>
