  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#9C9285] text-sm">{assets.length} assets · {assets.filter(a => a.status === "ACTIVE").length} active</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setFilterPillar("")} className={`text-xs px-3 py-1.5 rounded-full font-medium border transition-colors ${!filterPillar ? "bg-[#1C1714] text-white border-[#1C1714]" : "bg-white text-[#5C5248] border-[#E5DDD0] hover:border-[#B8901A]"}`}>All</button>
        {PILLARS.map(p => {
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search assets…" className="w-full pl-9 pr-4 py-2.5 border border-[#E5DDD0] rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#B8901A]/30 focus:border-[#B8901A] text-[#1C1714] placeholder:text-[#9C9285]" />
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                const pc = pillarColors[a.pillar] ?? { bg: "#F5F3EE", text: "#9C9285", dot: "#9C9285" };
                return (
                  <tr key={a.id} className="border-b border-[#EDE8E0] hover:bg-[#F5F3EE] transition-colors last:border-0">
                    <td className="px-5 py-3.5">
                      <p className="text-[#1C1714] text-sm font-semibold">{a.name}</p>
                      <p className="text-[#9C9285] text-xs font-mono">{a.code}</p>
                    </td>
                    <td className="px-5 py-3.5 text-[#5C5248] text-sm">{a.site.name}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-[#5C5248] text-sm">{a.assetType}</td>
                    <td className="px-5 py-3.5 text-center text-[#1C1714] text-sm font-medium">{a._count.sensors}</td>
                    <td className="px-5 py-3.5 text-center">
                      {a._count.alerts > 0 ? <span className="text-[#B91C1C] font-bold text-sm">{a._count.alerts}</span> : <span className="text-[#9C9285] text-sm">—</span>}
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5 justify-end">
                        <button onClick={() => openEdit(a)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#FEF7E6] text-[#9C9285] hover:text-[#B8901A] transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                        <button onClick={() => setDelId(a.id)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#FEF2F2] text-[#9C9285] hover:text-[#B91C1C] transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      <CrudModal title={modal === "edit" ? "Edit Asset" : "New Asset"} open={!!modal} onClose={() => setModal(null)}>
        <div className="space-y-4">
          ...
          <div className="flex gap-2 justify-end pt-2">
            ...
            <button onClick={save} disabled={saving} className="px-4 py-2 rounded-lg bg-[#1C1714] text-white text-sm font-semibold hover:bg-[#2D2420] disabled:opacity-50 transition-colors">{saving ? "Saving…" : modal === "edit" ? "Save Changes" : "Create Asset"}</button>
          </div>
        </div>
      </CrudModal>
