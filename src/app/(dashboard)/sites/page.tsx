    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[#0D1B35] text-2xl font-bold">Sites</h1>
            <p className="text-[#6378A0] text-sm">{sites.length} sites · {sites.filter(s => s.isActive).length} active</p>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search sites…" className="w-full pl-9 pr-4 py-2.5 border border-[#D9E2F0] rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#B8901A]/30 focus:border-[#B8901A] text-[#0D1B35] placeholder:text-[#6378A0]" />
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map(site => (
            <div key={site.id} className="bg-white border border-[#D9E2F0] rounded-xl p-5 space-y-4 hover:shadow-md hover:border-[#C6D0E8] transition-all">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-[#0D1B35] font-semibold">{site.name}</h3>
                    <p className="text-[#6378A0] text-xs">{site.code} · {site.city ?? site.address ?? "—"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <a href={`/sites/${site.id}`} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#FEF7E6] text-[#6378A0] hover:text-[#B8901A] transition-colors">
                    <ExternalLink className="w-3.5 h-3.5" />
                  </a>
                  <button onClick={() => openEdit(site)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#FEF7E6] text-[#6378A0] hover:text-[#B8901A] transition-colors">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => setDelId(site.id)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#FEF2F2] text-[#6378A0] hover:text-[#B91C1C] transition-colors">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="bg-[#F2F5FB] rounded-lg p-3 text-center">
                    <Icon className="w-4 h-4 text-[#6378A0] mx-auto mb-1" />
                    <p className="text-[#0D1B35] font-bold text-sm">{val}</p>
              ...
              <div className="flex items-center justify-between text-xs border-t border-[#E4EAF5] pt-3">
                <span className="text-[#6378A0]">{site.timezone}</span>
                <div className="flex items-center gap-1.5">
                  <div className={`w-2 h-2 rounded-full ${site.isActive ? "bg-[#166534]" : "bg-[#6378A0]"}`} />
                  <span className={site.isActive ? "text-[#166534] font-medium" : "text-[#6378A0]"}>{site.isActive ? "ACTIVE" : "INACTIVE"}</span>
                </div>
              </div>
            </div>
          ))}
